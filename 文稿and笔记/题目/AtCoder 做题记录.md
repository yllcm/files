...
MnZn 刚学 OI，请问这就是 Atcoder 吗。

首先判合法是容易的，直接 2-SAT 刻画条件即可。

然后怎么做呢？随一下直到出来不合法的解就好了！太牛了！

### *ARC161F Everywhere is Sparser than Whole (Judge)

这个题稍微好点。

首先回想一下最大密度子图怎么做，发现可以边向点连边，然后跑最大闭合子图。

看起来做完了，但是发现如果存在密度为 $d$ 的子图，上述算法会跑出来原图或者空图的平凡结果。

怎么做呢？可以考虑钦定一条边使得不为空图，这样只要判断是否为原图即可。不为原图的条件相当于割掉至少一个左部点，那么就转化为了可行边问题。同样，钦定一条边不选，只需要判断不为空图即可。

分析复杂度可以得到 $\mathcal{O}(m\sqrt{m})$ 的结果。

### ARC160D Mahjong

搞笑了，把条件看成 $a_i\leq m$ 了。不过判断条件推出来了，就假装是做出来了！！

考虑怎么判断！那么目标就是把所有数都变成 $k$ 的倍数，那么操作就是从前往后，每次以一个数为开头把后面的都加上一个数让它变成 $k$ 的倍数，然后继续往后面做。由于判断方法是唯一的，所以这个序列可以和下列操作序列构成双射：

* 对 $[i,i+k-1]$ 加上 $f_i<k$。
* 对 $i$ 加上 $g_ik$。

那么条件就是 $(\sum f_i+\sum g_i)k=m$，容斥然后插板就好了！！

### *ARC160E Make Biconnected

把条件转化错了！以为是覆盖除了叶子之外的所有点，每次选路径覆盖除了端点外的任何点！

那么从这里出发！发现如果有叶子结点没有操作那么它就寄了，那么可以考虑先匹配叶子结点。当叶子结点是偶数的时候是经典构造，当叶子结点是奇数的时候，猜测答案就是偶数的构造加上一条路径。枚举这个叶子，那么剩下的一个结点一定在这个叶子的二度链外。考虑下界是全局最小值，稍微想想就可以发现唯一的特例是一个根挂三条二度链，这种选次大值就好了。

### *ARC160F Count Sorted Arrays

好难。

考虑对于此类排序问题的套路：把 $\leq x$ 的标为 $0$，$>x$ 的标为 $1$。那么一个排列可以表示为 $n$ 维超立方体上的一条从 $(0,0,\cdots,0)$ 到 $(1,1,\cdots 1)$ 的格路，其中 $\text{popcount}=i$ 的数表示 $i$ 对应生成的 $01$ 序列。

所以一个排列的条件转化成格路上每个位置都是合法的！可以 DP 做到 $\mathcal{O}(2^nn)$。

但是 $m$ 有点大，但是实际上可以证明有效的操作 $(u,v)$ 是递减的，所以实际上可以看作 $m=\dfrac{n(n+1)}{2}$。